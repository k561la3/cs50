...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            char* xtmp=x;
            sscanf(line, "%s %s %s\r\n",x,y,z);
            while(x++){if(*x==0)break;
                if(!isalnum(*x)){error(400);printf("err almum");}
            }
            x=xtmp;
            if(y[0]!='/'){error(501);printf("root pls");}
           ...
           if(*y==0)break;
            ...
            if ( fgets (bufa , 1 , fl) == NULL ){error(501);printf("not readable");}
            fclose(fl);
